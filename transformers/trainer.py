...
from .optimization import AdamW, get_linear_schedule_with_warmup, get_constant_schedule, get_linear_schedule_with_warmup_to_min_lr
...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        #print('\nusing constant lr scheduler\n')
        #self.lr_scheduler = get_constant_schedule(self.optimizer, last_epoch = num_training_steps)

        #print ('\nusing get_linear_schedule_with_warmup_to_min_lr\n')
        #self.lr_scheduler = get_linear_schedule_with_warmup_to_min_lr(self.optimizer, num_warmup_steps = self.args.warmup_steps, num_training_steps = num_training_steps)
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                ## haiye: return loss from training
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if (
                        ...
                        ## haiye: return the loss for evaluation
        ...
        return TrainOutput(self.state.global_step, tr_loss.item() / self.state.global_step), training_logs
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        ## haiye: return the loss from training step
        if self.args.fp16 and _use_native_amp:
            with autocast():
    ...
    def prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            ### haiye: return loss for evaluation
            #loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only)
            ## haiye: 
            (loss, logits, labels), masks = self.prediction_step(model, inputs, prediction_loss_only)
            #print(masks)
            #eval_masks.append(masks.data.tolist()[0]) #append(masks.detach().cpu().numpy())
            eval_masks.append(masks.item())
        ...
        ## haiye:
        metrics['eval_masks'] =  np.mean(eval_masks)
    ...
    def prediction_step(
        ...
        return (loss, logits, labels), masks
